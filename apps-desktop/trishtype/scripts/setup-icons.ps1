# Phase 17.6 - Setup logo + OS icons for TrishType.

$repoRoot = Resolve-Path "$PSScriptRoot\..\..\.."
$srcLogo  = Join-Path $repoRoot "apps-desktop\trishlauncher\src\icons\trishtype.png"
$appRoot  = Join-Path $repoRoot "apps-desktop\trishtype"
$assetsDir = Join-Path $appRoot "src\assets"
$assetsLogo = Join-Path $assetsDir "logo.png"

Write-Host ""
Write-Host "==> TrishType - Setup icons" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $targetDir) {
    foreach ($mode in @("debug", "release")) {
        ...
        $exePath = Join-Path $modeDir "trishtype.exe"
        if (Test-Path $exePath) { Remove-Item $exePath -Force -ErrorAction SilentlyContinue }
        $buildOutDir = Join-Path $modeDir "build"
        if (Test-Path $buildOutDir) {
            Get-ChildItem $buildOutDir -Directory -Filter "trishtype-*" |
                ForEach-Object { Remove-Item $_.FullName -Recurse -Force -ErrorAction SilentlyContinue }
        }
        $depsDir = Join-Path $modeDir "deps"
        if (Test-Path $depsDir) {
            Get-ChildItem $depsDir -Filter "trishtype*" |
