...
export async function setUserRole(
  ...
  const body: Record<string, unknown> = { uid, role };
  if (role === 'demo' && typeof demoDays === 'number' && demoDays > 0) {
    body.demoDays = demoDays;
  }
  ...
  if (actor) {
    await writeAudit({
      ...
      details: { new_role: role, demo_days: role === 'demo' ? demoDays ?? 30 : null },
    });
  }
}
...
export async function fetchStats(): Promise<AdminStats> {
  ...
  const byRole: Record<UserRole, number> = { trial: 0, demo: 0, user: 0, admin: 0 };
  let signups7d = 0;
