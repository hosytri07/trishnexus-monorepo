 ...
 *
 * Sync content qua localStorage (TrishWork main + sticky window cùng đọc/ghi
 * cùng key 'trishwork.sticky.draft').
 */

import { useEffect, useRef, useState } from 'react';

const DRAFT_KEY = 'trishwork.sticky.draft';
...
export function StickyApp(): JSX.Element {
  ...
  // Phase 53.1 — Minimize xuống taskbar (giữ icon dưới taskbar để click mở lại)
  async function handleMinimize(): Promise<void> {
    try {
      const { getCurrentWindow } = await import('@tauri-apps/api/window');
      await getCurrentWindow().minimize();
    } catch (err) {
      console.warn('[sticky] minimize fail:', err);
    }
  }
    ...
    >
      ...
      >
        ...
        <button
          ...
          title="Ẩn (vẫn giữ nội dung — click 🗒 ở app chính để mở lại)"
          style={{
            border: 'none',
            background: 'transparent',
            cursor: 'pointer',
