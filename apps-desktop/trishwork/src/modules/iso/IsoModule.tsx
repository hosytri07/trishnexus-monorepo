...
function AppGate({ initialPage, hideNav }: { initialPage?: PageKey; hideNav?: boolean }): JSX.Element {
  ...
  if (!firebaseUser) {
    return null; // AuthGate above already handles login
  }
...
function MainApp({ initialPage, hideNav = false }: { initialPage?: PageKey; hideNav?: boolean }) {
  ...
  function attachFile(item: MucLucItem, e: ChangeEvent<HTMLInputElement>) {
    ...
    const newAttachment: TaiLieuDinhKem = {
      ...
      ghiChu: 'Lưu metadata cục bộ. Sau này sẽ upload file thật lên server.',
    };
