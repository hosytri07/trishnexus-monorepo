# Phase 17.5 - Setup logo + OS icons for TrishImage.

$repoRoot = Resolve-Path "$PSScriptRoot\..\..\.."
$srcLogo  = Join-Path $repoRoot "apps-desktop\trishlauncher\src\icons\trishimage.png"
$appRoot  = Join-Path $repoRoot "apps-desktop\trishimage"
$assetsDir = Join-Path $appRoot "src\assets"
$assetsLogo = Join-Path $assetsDir "logo.png"

Write-Host ""
Write-Host "==> TrishImage - Setup icons" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $targetDir) {
    foreach ($mode in @("debug", "release")) {
        ...
        $exePath = Join-Path $modeDir "trishimage.exe"
        if (Test-Path $exePath) { Remove-Item $exePath -Force -ErrorAction SilentlyContinue }
        $buildOutDir = Join-Path $modeDir "build"
        if (Test-Path $buildOutDir) {
            Get-ChildItem $buildOutDir -Directory -Filter "trishimage-*" |
                ForEach-Object { Remove-Item $_.FullName -Recurse -Force -ErrorAction SilentlyContinue }
        }
        $depsDir = Join-Path $modeDir "deps"
        if (Test-Path $depsDir) {
            Get-ChildItem $depsDir -Filter "trishimage*" |
...
Write-Host ""
Write-Host "[OK] Setup icons done. Run 'pnpm tauri dev' to test." -ForegroundColor Green
Write-Host ""
