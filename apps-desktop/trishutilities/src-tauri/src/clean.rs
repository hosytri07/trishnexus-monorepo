...
#[tauri::command]
pub fn list_clean_presets() -> Vec<CleanPreset> {
...
#[tauri::command]
pub fn list_trash_sessions() -> Result<Vec<TrashManifest>, String> {
    let trash = trash_root()?;
...
#[tauri::command]
pub fn restore_session(session_id: String) -> Result<usize, String> {
    let trash = trash_root()?;
    let session_dir = trash.join(&session_id);
...
#[tauri::command]
pub fn purge_session(session_id: String) -> Result<(), String> {
    let trash = trash_root()?;
    let session_dir = trash.join(&session_id);
    if !session_dir.exists() {
        return Err(format!("Session không tồn tại: {}", session_id));
...
#[tauri::command]
pub fn purge_old_sessions(retention_days: Option<u64>) -> Result<usize, String> {
...
#[tauri::command]
pub fn scan_autocad_junk() -> Result<ScanStats, String> {
...
/// add native dep nặng.
#[tauri::command]
pub fn disk_usage() -> Result<DiskInfo, String> {
