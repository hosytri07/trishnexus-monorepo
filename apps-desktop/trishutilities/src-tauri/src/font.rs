...
#[tauri::command]
pub async fn read_font(path: String) -> Result<FontMeta, String> {
    let p = Path::new(&path);
...
#[tauri::command]
pub async fn scan_system_fonts(
    max_entries: Option<usize>,
) -> Result<ScanFontsStats, String> {
    let cap = max_entries.unwrap_or(2_000).clamp(10, 10_000);
    let started = std::time::Instant::now();
...
#[tauri::command]
pub async fn download_and_install_pack(
    pack_id: String,
    url: String,
    sha256: String,
    app: tauri::AppHandle,
    ...
    // Phase 63 — Stream download để emit progress event
    let total_bytes = resp.content_length().unwrap_or(0);
    let start_time = std::time::Instant::now();
    let mut last_emit = std::time::Instant::now();
    let mut downloaded: u64 = 0;
    let mut bytes_vec: Vec<u8> = Vec::with_capacity(total_bytes as usize);
    ...
    while let Some(chunk_result) = stream.next().await {
        ...
        // Throttle progress event mỗi 200ms
        if last_emit.elapsed() >= std::time::Duration::from_millis(200) {
            last_emit = std::time::Instant::now();
            let elapsed_sec = start_time.elapsed().as_secs_f64();
            let speed_mb_per_s = if elapsed_sec > 0.0 {
                (downloaded as f64) / 1_048_576.0 / elapsed_sec
            } else {
                0.0
            };
            let eta_sec = if speed_mb_per_s > 0.0 && total_bytes > downloaded {
                ((total_bytes - downloaded) as f64) / 1_048_576.0 / speed_mb_per_s
            } else {
                0.0
            };
            let _ = app.emit(
    ...
    // Phase 63 fix: type annotation cho Cursor (bytes giờ là Vec<u8> sau khi stream)
...
#[tauri::command]
pub async fn list_pack_files(pack_id: String) -> Result<Vec<PackFileEntry>, String> {
    if pack_id.is_empty() || pack_id.contains("..") || pack_id.contains('/') || pack_id.contains('\\') {
        return Err(format!("pack_id không hợp lệ: {pack_id}"));
    }
    let appdata = std::env::var("APPDATA")
...
#[tauri::command]
pub fn detect_autocad_dirs() -> Vec<AutoCadInstall> {
    #[cfg(target_os = "windows")]
    {
...
#[tauri::command]
pub async fn install_shx_fonts(paths: Vec<String>) -> Result<Vec<ShxInstallResult>, String> {
...
#[tauri::command]
pub async fn export_fonts_to_folder(
    paths: Vec<String>,
    dest_dir: String,
...
#[tauri::command]
pub fn get_packs_folder_info() -> Result<PacksFolderInfo, String> {
    let appdata = std::env::var("APPDATA")
        .or_else(|_| std::env::var("HOME"))
        .map_err(|_| "Không tìm thấy APPDATA".to_string())?;
...
#[tauri::command]
pub fn clear_all_packs() -> Result<(), String> {
    let appdata = std::env::var("APPDATA")
        .or_else(|_| std::env::var("HOME"))
        .map_err(|_| "Không tìm thấy APPDATA".to_string())?;
    let packs_dir = std::path::PathBuf::from(&appdata)
