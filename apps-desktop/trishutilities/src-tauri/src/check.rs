...
fn collect_gpus_windows() -> Vec<GpuInfo> {
    ...
    // Wave 71.2 — Win32_VideoController.AdapterRAM là DWORD bị cap 4GB cho GPU
    // VRAM > 4GB (vd: RTX 5060 Ti 16GB chỉ báo 4GB). Phải đọc registry
    // HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e968-...}\* lấy
    // HardwareInformation.qwMemorySize (REG_QWORD) — chứa VRAM thực.
    //
    // PowerShell command: lấy Win32_VideoController cho name+driver, sau đó
    // merge với registry để lấy VRAM thực qua DriverDesc match.
    let ps_command = r#"
$g = Get-CimInstance Win32_VideoController -ErrorAction SilentlyContinue
$r = Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e968-e325-11ce-bfc1-08002be10318}\*' -ErrorAction SilentlyContinue
$out = @()
foreach ($vc in $g) {
  if (-not $vc.Name) { continue }
  $vram = if ($vc.AdapterRAM) { [int64]$vc.AdapterRAM } else { 0 }
  foreach ($rk in $r) {
    if ($rk.DriverDesc -and $rk.DriverDesc -eq $vc.Name) {
      $q = $rk.'HardwareInformation.qwMemorySize'
      $m = $rk.'HardwareInformation.MemorySize'
      if ($q -and [int64]$q -gt $vram) { $vram = [int64]$q }
      elseif ($m -and [int64]$m -gt $vram) { $vram = [int64]$m }
      break
    }
  }
  $out += [PSCustomObject]@{ Name = $vc.Name; AdapterRAM = $vram; DriverVersion = $vc.DriverVersion }
}
...
"#;
    ...
    parsed
        ...
        .filter_map(|v| {
            ...
            // AdapterRAM có thể là number (int64 từ registry), null
            let vram_bytes = v
                .get("AdapterRAM")
                .and_then(|x| {
                    // Có thể là number trực tiếp hoặc string nếu int64 quá lớn
                    x.as_u64().or_else(|| x.as_str().and_then(|s| s.parse::<u64>().ok()))
                })
                .unwrap_or(0);
...
#[tauri::command]
pub fn cpu_benchmark(rounds: Option<u32>) -> BenchResult {
...
#[tauri::command]
pub fn memory_bandwidth(rounds: Option<u32>) -> BenchResult {
...
#[tauri::command]
pub fn disk_benchmark(size_mb: Option<u32>) -> Result<DiskBenchResult, String> {
    use std::fs;
...
#[tauri::command]
pub fn top_processes(limit: Option<usize>) -> TopProcesses {
...
#[tauri::command]
pub fn scan_installed_software() -> Result<Vec<InstalledSoftware>, String> {
    #[cfg(target_os = "windows")]
    {
...
// ============================================================
// Wave 72.1 — Network speed test (Cloudflare /__down /__up endpoint)
// ============================================================

#[derive(Debug, Serialize)]
pub struct SpeedTestResult {
    /// Tốc độ download (Mbps — Megabits per second, chuẩn ISP)
    pub download_mbps: f64,
    /// Tốc độ upload (Mbps)
    pub upload_mbps: f64,
    /// Latency / ping trung bình (ms)
    pub latency_ms: f64,
    /// Jitter (độ lệch latency giữa các ping, ms)
    pub jitter_ms: f64,
    /// Bytes download thực sự (để debug)
    pub bytes_downloaded: u64,
    /// Bytes upload thực sự
    pub bytes_uploaded: u64,
    /// Tổng thời gian test (ms)
    pub total_ms: u64,
    /// Endpoint dùng (Cloudflare)
    pub server: String,
}
...
pub async fn network_speed_test() -> Result<SpeedTestResult, String> {
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| format!("Khởi tạo HTTP client lỗi: {}", e))?;

    let test_start = TokioInstant::now();

    // ── 1. Latency / Jitter: 5 ping HEAD đến /__down?bytes=0 ──────────
    let mut latencies: Vec<f64> = Vec::with_capacity(5);
    for _ in 0..5 {
    ...
    if latencies.is_empty() {
        return Err("Không kết nối được tới speed.cloudflare.com".to_string());
    }
    let mean_lat = latencies.iter().copied().sum::<f64>() / latencies.len() as f64;
    let jitter = if latencies.len() > 1 {
        let var = latencies
            .iter()
            .map(|v| (v - mean_lat).powi(2))
            .sum::<f64>()
    ...
    // ── 2. Download: stream 25 MB từ Cloudflare ──────────────────────
    // Cloudflare /__down trả random bytes nhanh, không cache, dùng đo speed
    // chuẩn (Cloudflare speedtest dùng endpoint này).
    let download_size: u64 = 25 * 1024 * 1024; // 25 MB
    let dl_url = format!(
        "https://speed.cloudflare.com/__down?bytes={}",
        download_size
    );
    let dl_start = TokioInstant::now();
    ...
    if !dl_resp.status().is_success() {
        return Err(format!("Download HTTP {}", dl_resp.status()));
    }
    let mut bytes_downloaded: u64 = 0;
    while let Some(chunk) = dl_resp
        .chunk()
        .await
        .map_err(|e| format!("Download stream lỗi: {}", e))?
    {
        bytes_downloaded += chunk.len() as u64;
    }
    let dl_elapsed = dl_start.elapsed().as_secs_f64().max(0.001);
    // bits/sec = bytes * 8 / sec  →  Mbps = bps / 1_000_000
    let download_mbps = (bytes_downloaded as f64 * 8.0) / dl_elapsed / 1_000_000.0;

    // ── 3. Upload: POST 8 MB body lên Cloudflare ─────────────────────
    let upload_size: usize = 8 * 1024 * 1024; // 8 MB (giảm cho ISP upstream chậm VN)
    let upload_body: Vec<u8> = vec![0u8; upload_size];
    let up_url = format!(
        "https://speed.cloudflare.com/__up?bytes={}",
        upload_size
    );
    let up_start = TokioInstant::now();
    let up_resp = client
        .post(&up_url)
        ...
        .send()
        .await;
    let (bytes_uploaded, upload_mbps) = match up_resp {
        Ok(r) if r.status().is_success() || r.status().is_redirection() => {
            let up_elapsed = up_start.elapsed().as_secs_f64().max(0.001);
            let mbps = (upload_size as f64 * 8.0) / up_elapsed / 1_000_000.0;
            (upload_size as u64, mbps)
        }
        _ => (0, 0.0),
    };
    ...
    Ok(SpeedTestResult {
        ...
        latency_ms: (mean_lat * 10.0).round() / 10.0,
