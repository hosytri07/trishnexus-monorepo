...
import { Download, Trash2, FileText, Image as ImgIcon, Film, Music, Archive, FileQuestion, Loader2, RefreshCw, AlertCircle, Share2, Copy, X, CheckCircle2 } from 'lucide-react';
...
export function FilesPage({ uid, search, refreshTick }: { uid: string; search: string; refreshTick: number }): JSX.Element {
  ...
  async function load() {
    ...
    setErr(null);
    try {
      const rows = await invoke<FileRow[]>('db_files_list', { folderId: null, search: null });
      setFiles(rows);
    } catch (e) {
      setErr(String(e));
    } finally {
  ...
  return (
    <div className="card">
      <div className="card-header">
        <div>
          <h2 className="card-title">File của tôi ({filtered.length})</h2>
          <p className="card-subtitle">
            Tất cả file đã upload — encrypted AES-256-GCM, lưu trong Telegram channel của bạn.
          </p>
        </div>
              ...
              {filtered.map(f => (
                <tr key={f.id}>
                  ...
                  <td>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>SHA {f.sha256_hex.slice(0, 8)}... · {f.total_chunks} chunk</div>
                  </td>
