/**
 * LoginScreen — TrishFinance Phase 23.1.B.
 * Pattern y hệt TrishISO/TrishDrive User app.
 */
...
const REMEMBER_KEY = 'trishfinance_remember_me';
    ...
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: 'var(--color-surface-bg)' }}>
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center gap-3 mb-6">
          <img src={logoUrl} alt="TrishFinance" style={{ width: 100, height: 100, borderRadius: 22, objectFit: 'cover', boxShadow: 'var(--shadow-md)' }} />
          <div className="text-center">
            <h1 style={{ fontSize: 22, fontWeight: 600, color: 'var(--color-text-primary)' }}>TrishFinance</h1>
            <p style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>Quản lý nhà trọ · Tài chính cá nhân · Bán hàng</p>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <p style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>
            {mode === 'signin'
              ? 'Login tài khoản TrishTEAM. Mỗi user có database riêng + sync Firestore Cloud.'
        ...
        <p style={{ fontSize: 11, color: 'var(--color-text-muted)', textAlign: 'center', marginTop: 16 }}>TrishFinance · TrishTEAM ecosystem</p>
      </div>
    </div>
  );
}
...
function prettyError(msg: string): string {
  ...
  // Bỏ prefix "Firebase: Error" nếu rớt qua tới đây
