...
/**
 * /tieng-anh — Phase 19.20 — MVP English quiz.
 *
...
import {
  ...
  type EnglishExamResult,
  ENGLISH_TOPIC_CONFIGS,
  buildEnglishExam,
  evaluateEnglishExam,
...
const STORAGE_KEY = 'trishteam:english-exam';

export default function EnglishExamPage() {
  const [view, setView] = useState<View>('picker');
  const [exam, setExam] = useState<ExamState | null>(null);
  const [result, setResult] = useState<EnglishExamResult | null>(null);
  ...
  function start(topic: EnglishTopic) {
    const questions = buildEnglishExam(topic);
  ...
  function submit() {
    if (!exam) return;
    setResult(evaluateEnglishExam(exam.topic, exam.questions, exam.answers));
    setView('result');
    try {
...
function PickerView({ onStart }: { onStart: (t: EnglishTopic) => void }) {
  return (
    <main className="max-w-4xl mx-auto px-6 py-10">
      ...
      <header className="mb-8">
        <div className="flex items-center gap-3 mb-3">
          <Languages size={28} strokeWidth={1.75} style={{ color: 'var(--color-accent-primary)' }} />
        ...
        <p className="text-base md:text-lg max-w-2xl" style={{ color: 'var(--color-text-secondary)' }}>
          Chọn chuyên đề để luyện tập. Bộ câu hỏi mẫu tập trung kỹ thuật xây dựng / TOEIC — bộ đầy đủ IELTS / 6 bậc VN sẽ bổ sung.
        </p>
      </header>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {(Object.keys(ENGLISH_TOPIC_CONFIGS) as EnglishTopic[]).map((topic) => {
          const cfg = ENGLISH_TOPIC_CONFIGS[topic];
          return (
            <button
              key={topic}
              type="button"
            ...
            >
              <div className="flex items-center gap-3 mb-3 w-full">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span
                      className="text-[10px] font-bold px-1.5 h-5 rounded inline-flex items-center"
              ...
              <div className="grid grid-cols-3 gap-2 w-full text-xs" style={{ color: 'var(--color-text-secondary)' }}>
                <Stat label="Questions" value={`${cfg.questionCount}`} />
                <Stat label="Pass ≥" value={`${cfg.passingScore}/${cfg.questionCount}`} />
                <Stat label="Time" value={`${cfg.durationMin}'`} />
              </div>
...
function QuizView({ exam, onAnswer, onPrev, onNext, onJumpTo, onSubmit, onReset }: QuizProps) {
  const cfg = ENGLISH_TOPIC_CONFIGS[exam.topic];
    ...
    <main className="max-w-5xl mx-auto px-6 py-6">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold" style={{ color: 'var(--color-text-primary)' }}>
              {cfg.name} — Question {exam.currentIdx + 1}/{exam.questions.length}
            </div>
        ...
        <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--color-text-muted)' }}>
          <Clock size={14} /> {elapsedMin} / {cfg.durationMin} min
        </div>
        <button
          type="button"
          onClick={() => {
            if (window.confirm('Quit exam? Progress will be lost.')) onReset();
          }}
      ...
      >
        {q.passage && (
          <div
            className="rounded-md p-3 mb-4 text-sm leading-relaxed"
            style={{
          ...
          >
            <div className="text-[10px] uppercase tracking-wider font-bold mb-1.5" style={{ color: cfg.color }}>
              📰 Reading passage
            </div>
            <p>{q.passage}</p>
          </div>
          ...
          <button
            ...
            onClick={() => {
              if (answeredCount < exam.questions.length) {
                if (!window.confirm(`You have ${exam.questions.length - answeredCount} unanswered question(s). Submit anyway?`)) return;
              }
              onSubmit();
            }}
    ...
    <main className="max-w-4xl mx-auto px-6 py-10">
      ...
      >
        ...
        <p className="text-sm mb-3" style={{ color: 'var(--color-text-secondary)' }}>
          {cfg.name} ({cfg.level}) — pass ≥ {result.passingScore}/{result.total}
        </p>
        <div className="mt-3">
          <button
            type="button"
      ...
      <section>
        <h2 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: 'var(--color-text-muted)' }}>
          Detailed answers ({wrongs.length} wrong)
        </h2>
...
function ReviewCard({ index, detail }: { index: number; detail: EnglishExamResult['details'][number] }) {
    ...
    >
      <div className="flex items-start gap-2 mb-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {question.passage && (
            <p
              className="text-xs mb-2 italic line-clamp-2"
